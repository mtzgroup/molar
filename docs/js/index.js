...
const getModelURL = window.getModelURL;

(() => {
  const modelPath = "/molar";
  ...
  function showAR(event) {
    event.preventDefault();
    const cell = this.getAttribute("data-name");
    getModelURL(cell.replaceAll("_", " "), "usdz").then(url => {
  ...
  function createLink(molecule) {
    ...
    if (isAndroid) {
      ...
      a.addEventListener("click", showAR);
    } else {
      a.href = "#";
      a.setAttribute("data-name", cell);
    ...
    img.src = modelPath + "/img/blank.png";
    getModelURL(cell.replaceAll("_", " "), "png").then(url => {
      img.src = url;
      hideSpinner();
    }).catch(hideSpinner);
